  ...
  return (
    <div ref={panelRef} style={{ position: 'relative' }} className="tooltip-wrapper">
      ...
      <button
        ...
        style={{ position: 'relative', background: 'none', border: theme === 'light' ? '1.5px solid #32438c' : 'none', cursor: 'pointer', color: open ? 'var(--blue-400)' : theme === 'light' ? '#32438c' : 'var(--text-secondary)', padding: '0.4rem', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '8px', transition: 'all 0.2s' }}>
        <Bell size={18} color={theme === 'light' ? '#32438c' : 'var(--text-secondary)'} fill='none' strokeWidth={2} />
        {unreadCount > 0 && (

...
export default function Settings() {
  ...
  const [settings, setSettings] = useState({ company_name: '', primary_color: '#2563eb', careers_page_url: '', sender_email: '', sender_name: '' })
  ...
  useEffect(() => {
    if (dbSettings) setSettings({ company_name: dbSettings.company_name || '', primary_color: dbSettings.primary_color || '#2563eb', careers_page_url: dbSettings.careers_page_url || '', sender_email: dbSettings.sender_email || '', sender_name: dbSettings.sender_name || '' })
  }, [dbSettings])
      ...
      {activeTab === 'company' && (
        <div className="card" style={{ maxWidth: '600px' }}>
          ...
          <div className="form-group">
            ...
            <input className="input" value={settings.careers_page_url} onChange={e => setSettings({ ...settings, careers_page_url: e.target.value })} placeholder="https://yourcompany.com/careers" />
          </div>
          ...
          <div className="form-group">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              <input type="color" value={settings.primary_color} onChange={e => setSettings({ ...settings, primary_color: e.target.value })} style={{ width: '48px', height: '40px', border: 'none', borderRadius: '8px', cursor: 'pointer', background: 'none' }} />
